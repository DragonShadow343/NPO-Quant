...
const Dashboard = () => {
  ...
  const { carbon = [], accounting = [], grants = [], category_totals = {} } = data;
  ...
  const latestGrant      = grants.length     ? grants[grants.length - 1]           : { eligibility_percent: 0 };
  const prevGrant        = grants.length > 1 ? grants[grants.length - 2]           : null;
  ...
  const grantTrend = prevGrant
    ? (latestGrant.eligibility_percent - prevGrant.eligibility_percent).toFixed(1)
    : 0;

  const currentEmissions = latestCarbon.total_emissions;
  const currentProfit = latestAccounting.profit_loss;
  const grantEligibility = latestGrant.eligibility_percent;
        ...
        <div className="max-w-6xl mx-auto mt-10 space-y-8">
          ...
          <div className="grid grid-cols-4 gap-6">
            ...
            <div className="bg-white border rounded-2xl p-6 shadow-sm">
              ...
              <div className="flex items-center gap-2">
                <h3 className="text-2xl font-semibold">{grantEligibility}%</h3>
                <span className={`text-sm font-semibold ${grantTrend > 0 ? "text-green-600" : "text-red-500"}`}>
                  {grantTrend > 0 ? "↑" : "↓"} {Math.abs(grantTrend)}
                </span>
              </div>
            </div>
            <div
          ...
          <div className="grid grid-cols-6 gap-8">
            ...
            <div className="bg-gray-100 rounded-3xl p-10 border col-span-4">
              <h2 className="text-xl font-semibold mb-4 text-center">Grant Eligibility</h2>
              <LineChart width={600} height={300} data={grants}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="month" />
                <YAxis />
                <Tooltip />
                <Legend />
                <Line type="" dataKey="eligibility_percent" stroke="#ffc658" name="% Eligible" />
              </LineChart>
            </div>
                  ...
                  <Pie
                    data={
                      carbon[carbon.length - 1]?.composition
                        ? Object.entries(carbon[carbon.length - 1].composition).map(([name, value]) => ({ name, value }))
                        : []
                    }
                    dataKey="value"
                    nameKey="name"
            ...
            <div className="bg-gray-100 rounded-3xl p-10 border col-span-3">
              ...
              <div className="flex justify-between items-center mt-4">
                <button
                  className="px-3 py-1 bg-gray-300 rounded"
                  onClick={() => setFinanceSlide((financeSlide - 1 + 2) % 2)}
                >
                  ←
                </button>
                ...
                <button
                  className="px-3 py-1 bg-gray-300 rounded"
                  onClick={() => setFinanceSlide((financeSlide + 1) % 2)}
                >
                  →
                </button>
              </div>
